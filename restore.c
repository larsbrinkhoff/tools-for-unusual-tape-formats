...
static int tape_records = 0;
static int tape_blocks = 0;
  ...
  while (rptr >= rend)
    {
      ...
      if (n == 0)
        {
          fprintf (info, "Tape mark.\n");
          if (eof == 2)
            fprintf (info, "Logical end of tape.\n");
        }
      else if ((n >> 24) == 0xFF)
        fprintf (info, "End of medium.\n");
      else if (n & 0x80000000)
        fprintf (info, "Tape error.\n");
      else
        {
          tape_records++;
          rptr = record;
...
detect_format (uint8_t *image)
{
  if (memcmp (image + 18, "\x6B\xEA", 2) == 0)
    ...
    {
      fprintf (info, "Little endian 32-bit new dump.\n");
      big_endian = 0;
      long_word = 1;
    ...
    {
      fprintf (info, "Little endian 32-bit old dump.\n");
      big_endian = 0;
      long_word = 1;
      word_mask = 0xFFFFFFFF;
      magic = 60011;
    ...
    {
      fprintf (info, "Big endian 32-bit new dump.\n");
      big_endian = 1;
      long_word = 1;
      word_mask = 0xFFFFFFFF;
      magic = 60012;
    ...
    {
      fprintf (info, "Big endian 32-bit old dump.\n");
      big_endian = 1;
      long_word = 1;
      word_mask = 0xFFFFFFFF;
      magic = 60011;
  ...
  if (h_magic == magic)
    {
      ...
      if (tape_volume != h_volume)
        fprintf (stderr, "Incorrect volume; expected %d, got %d.\n", tape_volume, h_volume);
...
mount_next (void)
{
  fprintf (info, "Physical end of tape.\n");
  fprintf (info, "Volume records: %d.\n", tape_records);
  fprintf (info, "Volume blocks: %d.\n", tape_blocks);
  fputc ('\n', info);
  fclose (tape);
  ...
  fprintf (info, "Reading from %s\n", *tape_argv);
  tape_blocks = tape_records = 0;
  tape_changed = 1;
  tape_argv++;
  return tape;
}
...
static void
write_file (char *name, inode_info *inode)
{
  char old[256];
  if (!extract)
    return;
  sprintf (old, ".inodes/%u", inode->inode);
  if (*name == '/')
    name++;
  mkdirs (name);
  if (inode->mode & 040000)
    mkdir (name, inode->mode & 0777);
  else
    {
      link (old, name);
      chmod (name, inode->mode & 0777);
      printf (">>> %s %s %o\n", old, name, inode->mode);
    }
}
...
usage (const char *x)
{
  fprintf (stderr, "Usage: %s -t|-x [-v] files...\n", x);
  exit (1);
}
...
main (int argc, char **argv)
{
  ...
  while ((opt = getopt (argc, argv, "tvxC:")) != -1)
    {
      switch (opt)
	{
	case 'v':
  ...
  list = verbose >= 1 ? stdout : fopen ("/dev/null", "w");
  info = verbose >= 2 ? stdout : fopen ("/dev/null", "w");
  debug = verbose >= 3 ? stdout : fopen ("/dev/null", "w");

  tape_argv = argv + optind + 1;
  tape_argc = argc - optind - 1;

  end_of_tape (mount_next);
  mkdir (".inodes", 0700);
  if (setjmp (jb) == 0)
    process_tapes (argv[optind]);
  ...
  fprintf (info, "Total number of directories: %d\n", directories);
  fprintf (info, "Total number of file names: %ld\n", fptr - filenames);
}
